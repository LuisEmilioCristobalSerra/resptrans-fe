<template>
  <body class="pt-5">
    ...
    >
      ...
      <div>
        <span
          >Sirva éste como comprobante de entrega de los siguientes equipos:
        </span>
        <div class="mt-3">
          <ul>
            <div v-for="item in props.items" :key="item">
              <li v-for="detail in item.items" :key="detail.id">
                <span
                  >{{ item.name }} {{ item.description }}
                  <span class="fw-bold fst-italic"> COD:</span>
                  {{ detail.code }}, <span class="fw-bold">SN:</span>
                  {{ detail.serial }}.</span
                >
              </li>
            </div>
        ...
        <div style="text-align: justify" class="mb-2">
          Pendiente entrega de responsivas individuales de los equipos.
        </div>
        <div style="text-align: justify" class="mb-2">
